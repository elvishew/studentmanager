...
3. **新建课程规划按钮**
   - 点击弹出多步骤创建对话框
   - 支持选择目标、课时数量、检测模板、编辑蓝图
   - 选择目标后创建课程规划（带默认配置或自定义配置）

4. **删除课程规划**
   - 点击删除按钮弹出二次确认对话框
   - 确认后删除课程规划及其所有课时和训练记录
   - 删除成功后显示 SnackBar 提示
...
点击"新建"按钮
    ↓
显示 CreateCoursePlanDialog（多步骤）
    ↓
步骤1: 选择课程目标
    ↓
步骤2: 选择课时数量（12/24/33/55/自定义）
    ↓
步骤3: 检测模板数据
    ↓
步骤4: 编辑蓝图（可选）
...
3. **新建课程规划** - 测试创建流程（多步骤）
4. **删除课程规划** - 测试删除功能及二次确认
5. **删除有课时的课程规划** - 验证级联删除
6. **多个课程规划** - 查看有多个课程规划的学员
7. **加载中状态** - 测试加载过程
8. **错误状态** - 模拟网络错误
9. **点击课程规划卡片** - 验证跳转到课程规划详情页
