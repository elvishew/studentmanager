...
└── Body (ListView with RefreshIndicator)
    └── Dismissible SessionTile × N
        ├── CircleAvatar (显示序号: 索引+1)
        ├── Title: 第 X 节课
        ├── Subtitle
        │   ├── 状态标签
        │   └── 上课时间
...
7. **⚠️ 课时序号显示** - 课时列表使用**列表索引+1**显示序号，而不是数据库中的 `session_number`。这样做的好处是：
   - 删除课时时序号会自动调整（如：删除第3节后，第4节自动显示为第3节）
   - 避免出现跳号现象
   - 不需要修改数据库的 `session_number` 字段
   - UI显示更加连续和友好
...
7. **下拉刷新** - 测试刷新功能
8. **错误状态** - 模拟加载失败
9. **滑动删除课时** - 向左滑动删除，测试二次确认对话框
10. **详情页删除课时** - 点击AppBar删除按钮测试删除功能
11. **删除后课时序号** - 验证删除后序号自动调整（如：删除第3节后，第4节显示为第3节）
12. **删除有训练内容的课时** - 验证级联删除训练块
